...
func NewFSock(fsaddr, fspaswd string, reconnects int, eventHandlers map[string][]func(string, string), eventFilters map[string][]string, l *logs.BeeLogger, connId string) (fsock *FSock, err error) {
	fsock = &FSock{
...
func NewFSockPool(maxFSocks int, fsaddr, fspasswd string, reconnects int, maxWaitConn time.Duration,
	...
	eventHandlers map[string][]func(string, string), eventFilters map[string][]string, l *logs.BeeLogger, connId string) (*FSockPool, error) {
	pool := &FSockPool{
		connId:        connId,
		fsAddr:        fsaddr,
		fsPasswd:      fspasswd,
